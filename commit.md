...
- 첫 줄 맨 앞에 관련 이슈 번호(`#이슈번호`)를 반드시 남깁니다.
...
1. **로컬 훅 (`commit-msg`)** — 저장소 클론 후 `npm install`을 실행하면 `prepare` 스크립트가 훅 경로를 자동으로 등록하여, 형식에 맞지 않는 커밋은 로컬에서 즉시 거부됩니다. (수동으로 등록하려면 `git config core.hooksPath .githooks` 실행)
